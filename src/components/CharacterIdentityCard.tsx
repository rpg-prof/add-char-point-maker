    ...
    <div className="dark-panel rounded-xl overflow-hidden border border-gold/25 shadow-sm">
      <div className="px-4 pt-4 pb-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="font-display text-micro tracking-[0.2em] uppercase text-gold/55 mb-1">
            Ficha de Personagem
          </p>
          <h3 className="font-display text-stat tracking-wide text-gold leading-tight truncate">
            {displayName}
          </h3>
          {playerName.trim() && (
            <p className="text-meta font-body text-parchment/55 mt-1 flex items-center gap-1.5">
        ...
        <div className="shrink-0 text-right rounded-lg border border-gold/25 bg-gold/10 px-3 py-2">
          <p className="font-display text-micro tracking-wider uppercase text-gold/70">Nível</p>
          <p className="font-display text-stat font-bold text-gold tabular-nums leading-none">
            {characterLevel}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-gold/10 mt-3">
        {headerFacts.map(({ label, value, hint }) => (
          <div key={label} className="bg-parchment-dark/40 px-3 py-2.5 min-w-0">
            <p className="font-display text-micro tracking-wider uppercase text-parchment/45 truncate">
              {label}
            </p>
            <p
              className="font-display text-meta text-parchment/90 leading-snug truncate mt-0.5"
              title={hint}
            >
              {value}
            </p>
      ...
      {physicalFacts.length > 0 && (
        <div className="px-4 py-2.5 flex flex-wrap gap-x-4 gap-y-1 border-t border-gold/10">
          {physicalFacts.map(({ label, value }) => (
            <span key={label} className="text-meta font-body text-parchment/60">
              <span className="font-display tracking-wide text-parchment/40 uppercase text-micro mr-1">
                {label}:
              </span>
              {value}
            </span>
