...
export type MagicAccessSection = "divine" | "arcane";

interface MagicAccessPanelProps {
  section: MagicAccessSection;
...
const clearBtnCls =
  "flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-display tracking-wider border border-border text-muted-foreground hover:border-red-400/50 hover:text-red-300 transition-all disabled:opacity-30 disabled:cursor-not-allowed";
          ...
          return (
            <div
              ...
              className={`flex items-center justify-between gap-2 px-3 py-2 rounded border transition-all ${
                current !== "none" ? "border-gold/50 bg-gold/10" : "border-border bg-card/40"
              }`}
            >
            ...
            <div className="flex-1 min-w-0">
              ...
              {school.specialization && (
                <div className="text-[10px] text-muted-foreground font-body truncate">
                  Esp.: {school.specialization.title} • Req.: {school.specialization.requirement} • Opostas:{" "}
                  {school.specialization.opposed.join(", ")}
                </div>
              )}
            </div>
